...
function __generator(thisArg, body) {
    ...
    function step(op) {
        if (f) throw new TypeError("生成器已在执行。");
        while (_) try {
...
var MyPlugin = (
        ...
        function() {
            ...
            function(_a) {
                ...
                this.addCommand({
                    id: 'selection-link',
                    name: '转换内链语法',
                    callback:function () { return _this.转换内链语法(); },
                    hotkeys: [
		                {
		                    modifiers: ["Alt"],
		                    key: "Z",
    ...
    MyPlugin.prototype.转换内链语法 = function() {
        ...
        if (!lines) return;
        var re = /[\"\|\[\]?\\\*<>\/:]/gi;	//是否包含[]()及标点符号
        var link0 = /\[\[|\]\]/g;
        var link1 = /^([^\[\]]*)!\[+([^\[\]]*)$|^([^\[\]]*)\[+([^\[\]]*)$|^([^\[\]]*)\]+([^\[\]]*)$/gmi;	//是否只包含一侧的[[或]]
  		var link2 = /^[^\[\]]*(\[\[[^\[\]\.]*\]\][^\[\]]*)+$/gmi;	//是否包含N组成对的内链语法
  		var link3 = /、/g;
	  	if (re.test(lines)) {
	  		if (link1.test(lines)){
	  			return;
