...
开发：蚕子 QQ：312815311 更新时间：2022-3-23
***************************************************************************** */
...
var 当前版本 = '0.4.6';
var 功能更新 = '此版本增加了一键调高或调低当前行标题级别、在文本行中插入制表符、计算所选四则算式的结果等功能！'
...
class MyPlugin extends obsidian.Plugin {
    ...
    onload() {
        ...
        this.addCommand({
            id: 'text-Color1',
            name: '转换橙色文字',
            callback: () => this.转换文字颜色(this.hColor1),
            hotkeys: [{ modifiers: ["Mod","Shift"], key: "1" } ]
        });
        this.addCommand({
            id: 'text-Color2',
            name: '转换红色文字',
            callback: () => this.转换文字颜色(this.hColor2),
            hotkeys: [{ modifiers: ["Mod","Shift"], key: "2" } ]
        });
        this.addCommand({
            id: 'text-Color3',
            name: '转换紫色文字',
            callback: () => this.转换文字颜色(this.hColor3),
            hotkeys: [{ modifiers: ["Mod","Shift"], key: "3" } ]
        });
        this.addCommand({
            id: 'text-Color4',
            name: '转换蓝色文字',
            callback: () => this.转换文字颜色(this.hColor4),
            hotkeys: [{ modifiers: ["Mod","Shift"], key: "4" } ]
        });
        this.addCommand({
            id: 'text-Color5',
            name: '转换绿色文字',
            callback: () => this.转换文字颜色(this.hColor5),
            hotkeys: [{ modifiers: ["Mod","Shift"], key: "5" } ]
        });
        this.addCommand({
            id: 'text-background1',
            name: '转换橙色背景',
            callback: () => this.转换背景颜色(this.hColor1),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "1" } ]
        });
        this.addCommand({
            id: 'text-background2',
            name: '转换红色背景',
            callback: () => this.转换背景颜色(this.hColor2),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "2" } ]
        });
        this.addCommand({
            id: 'text-background3',
            name: '转换紫色背景',
            callback: () => this.转换背景颜色(this.hColor3),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "3" } ]
        });
        this.addCommand({
            id: 'text-background4',
            name: '转换蓝色背景',
            callback: () => this.转换背景颜色(this.hColor4),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "4" } ]
        });
        this.addCommand({
            id: 'text-background5',
            name: '转换绿色背景',
            callback: () => this.转换背景颜色(this.hColor5),
        ...
        document.addEventListener('keydown',(e) =>{
            ...
            if (e.key == "Enter"){
                ...
                if(聚焦编辑){
                    ...
                    //console.log(this.twoEnter+" 缓存"+在列表行);
                    var reg1 = /^[\t ]+$/m;
                    var reg2 = /^[\t ]*\/\//m;
                    ...
                    }else if(this.twoEnter && !代码块内 && 在列表行<0){
        ...
        this.registerEvent(this.app.workspace.on('file-open', (file) => {
            if (file && file.path) {
                ...
                if(this.version!=当前版本){
                    const noticeContent = createFragment();
                    noticeContent.createEl("p").innerHTML = "<b>欢迎使用 ZH增强编辑 v"+当前版本+" 插件！</b>";
                    noticeContent.createEl("p").innerHTML = 功能更新;
                    noticeContent.createEl("button").innerHTML = "开发：<a href='http://wpa.qq.com/msgrd?v=3&uin=312815311&site=qq&menu=yes'>蚕子（QQ：312815311）</a>";
                    noticeContent.createEl("p").innerHTML = "点击此处 可关闭提示窗口......";
                    new obsidian.Notice(noticeContent, 0);
                    this.plugin.settings.version = 当前版本;
                    this.saveSettings();
                };
                */
            }
...
class editSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        containerEl.empty();
        containerEl.createEl('h2', { text: 'ZH增强编辑'+当前版本 });
        ...
        new obsidian.Setting(containerEl)
            .setName('📣 插入制表符「Tab」 在文本行中插入制表符效果')
            .setDesc('启用此项后，在文本行中按下 Tab 键会插入4个空格，不再整行缩进。')
        ...
        new obsidian.Setting(containerEl)
            .setName('📣 键控光标跳转「Alt+Shift +J +L」')
            .setDesc('控制光标在标题、列表项、待办项、代码块、引用等文本行或加粗、高亮、注释、删除、链接等MarkDown语法间前后跳转')
        ...
        new obsidian.Setting(containerEl)
            ...
            .setDesc('启用后，当未选文本时按下Alt +C加粗 +G高亮 +S删除线 +U上标 +N下标 等快捷键，即开启或关闭 MD语法「格式刷」功能。')
