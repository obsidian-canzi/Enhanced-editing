...
class MyPlugin extends obsidian.Plugin {
    ...
    转换潜在链接() {
        ...
        if (累计 < 1) {
            ...
            笔记正文 = 笔记正文.replace(/⚘/g,"");
            this.替换笔记正文(笔记正文);
            new obsidian.Notice("📣 下列 "+累计+"个潜在链接被转换：\n"+转换链接内容.join("\n")+"\n\n📣 可以使用 Ctrl+Z 撤消。");
        }
    };
...
class editSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        */
        ...
        mdText.appendChild(document.createElement('br'));
        mdText.appendText('调高所有标题级别「待设置」：将所有标题的级别调高一级（最高为一级）。此功能应 空（1977878681）的建议；');
        mdText.appendChild(document.createElement('br'));
        mdText.appendText('调低所有标题级别「待设置」：将所有标题的级别调低一级（最低为六级）。此功能应 空（1977878681）的建议；');
        mdText.appendChild(document.createElement('br'));
